...
const Client = () => {
  ...
  const { data, isPending } = useGetOrderTrack({ orderId: orderId as string });
              ...
              {histories.map((item) => (
                ...
                >
                  <TimelineHeader>
                    <TimelineSeparator
                      className={cn(
                        "group-data-[orientation=vertical]/timeline:-left-7 group-data-[orientation=vertical]/timeline:h-[calc(100%-1.5rem-0.25rem)] group-data-[orientation=vertical]/timeline:translate-y-6.5 !bg-gray-300",
                    ...
                    <TimelineIndicator
                      className={cn(
                        "group-data-completed/timeline-item:bg-primary group-data-completed/timeline-item:text-primary-foreground flex size-6 items-center justify-center border-none group-data-[orientation=vertical]/timeline:-left-7 !bg-gray-300 !text-black",
                        (item.status === "CONFIRMED" ||
                          item.status === "ALLOCATED") &&
                          "!bg-blue-300",
                        item.status === "DELIVERED" && "!bg-green-300",
