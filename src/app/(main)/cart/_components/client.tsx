...
const Client = () => {
  ...
  // Calculate qtyMap from carts - use as source of truth
  const qtyMapFromCarts = useMemo(() => {
    if (!carts) return {};
  ...
  // Track user modifications separately
  const [userModifications, setUserModifications] = useState<
    Record<string, number>
  >({});

  // Merge qtyMapFromCarts with user modifications, filter out deleted items
  const localQty = useMemo(() => {
    const merged = { ...qtyMapFromCarts };
    Object.keys(userModifications).forEach((key) => {
      // Only apply user modifications if the item still exists in carts
      if (key in qtyMapFromCarts) {
        merged[key] = userModifications[key];
      }
    });
    return merged;
  }, [qtyMapFromCarts, userModifications]);
  ...
  useEffect(() => {
    Object.entries(debouncedLocalQty).forEach(([variantId, qty]) => {
      ...
      if (qty !== originalQty && qty > 0 && !isTyping) {
        updateQty({
          params: { variantId },
          body: { qty: String(qty) },
        });
  ...
  const handleIncrease = (variantId: string) => {
    setUserModifications((prev) => ({
      ...prev,
      [variantId]: (localQty[variantId] ?? 0) + 1,
    }));
  };

  const handleReduce = (variantId: string) => {
    const updated = (localQty[variantId] ?? 0) - 1;
    if (updated <= 0) {
      setDialog(variantId);
      return; // jangan ubah qty dulu
    }
    setUserModifications((prev) => ({
      ...prev,
      [variantId]: updated,
  ...
  const handleQtyChange = (variantId: string, value: string) => {
    setUserModifications((prev) => ({
      ...prev,
      [variantId]: Number.parseFloat(numericString(value)),
    }));
  };
    ...
    <div className="bg-sky-50 h-full relative">
      ...
      <DialogRemoveProduct
        ...
        onSubmit={() => {
          ...
          setDialog("");
          // Remove from userModifications when deleted
          setUserModifications((prev) => {
            const updated = { ...prev };
