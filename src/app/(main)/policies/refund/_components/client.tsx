...
import { useGetRefundPolicy } from "../../_api";

export const Client = () => {
  const { data: refund, isPending } = useGetRefundPolicy();

  const refundRes = useMemo(() => refund?.data ?? "", [refund]);
  return (
    <div className="w-full flex flex-col gap-4 rounded-l p-3 lg:p-5 bg-white shadow-sm">
      {isPending ? (
