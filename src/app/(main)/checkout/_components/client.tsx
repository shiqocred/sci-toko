...
import { ArrowLeft, BadgePercent, Loader2, NotebookPen, X } from "lucide-react";
...
export default function Client() {
  ...
  const totalPrice = useMemo(() => {
    const shippingCost = checkout?.freeShipping
      ? 0
      : Number.parseFloat(shippingPrice);
  ...
  const isLoading =
    isPendingAddresses || isPendingCheckout || isPendingOngkir || isCreating;
  ...
  const handleOrder = async () => {
    ...
    createOrder(
      ...
      { onSuccess: ({ data }) => router.push(data.data.payment_url) }
    );
  };
